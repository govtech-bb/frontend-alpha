---
title: "Get a Primary School Textbook Grant"
stage: "alpha"
publish_date: 2025-12-11
---

You should complete your application in one go. At the moment, it is not possible to save your answers and come back to them later.

## How long does it take?

It should take no longer than 10 minutes to complete the registration form.

## What you will need

Before you start, you will need to know:

1. The student(s)' name, ID number and class.
2. The name of the child/children’s school and principal.
3. The details of a bank account that has been used within the last 3 months. If the bank account is frozen, the funds will not reach you.

<!-- Add custom html button to allow necessary styling -->
<!-- TODO: Make this file MDX so that the Button component can be imported -->

<a href="/money-financial-support/get-a-primary-school-textbook-grant/form" className='relative inline-flex items-center justify-center gap-2 rounded-sm text-body whitespace-nowrap transition-[background-color,box-shadow] duration-200 outline-none disabled-state px-6 py-4 bg-teal-dark text-neutral-white! hover:bg-brand-teal-light hover:shadow-[inset_0_0_0_4px_rgba(222,245,246,0.10)] active:bg-brand-teal-darker active:shadow-[inset_0_0_0_3px_rgba(0,0,0,0.20)] active:outline-none active:ring-4 active:ring-teal-100 active:ring-offset-1 active:rounded-sm focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-teal-100 focus-visible:ring-offset-1 focus-visible:rounded-sm no-underline' target="_self">Start</a>
