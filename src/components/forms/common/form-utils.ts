...
export function getFieldClassName(
  ...
  const baseClass =
    "w-full max-w-md rounded-md border-2 bg-white px-3 py-2 text-black-00 transition-all focus:border-[#1E787D] focus:ring-2 focus:ring-[#1E787D]/20";
...
export function getTextareaClassName(
  ...
  const baseClass =
    "w-full max-w-md resize-y rounded-md border-2 bg-white px-3 py-2 text-black-00 transition-all focus:border-[#1E787D] focus:ring-2 focus:ring-[#1E787D]/20";
  const errorClass = fieldErrors[field] ? "border-red-600" : "border-gray-300";
  return `${baseClass} ${errorClass}`;
}
