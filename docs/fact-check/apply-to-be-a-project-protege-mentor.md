...
- **Summary:** 13 claims reviewed — 6 verified, 2 discrepant, 5 unverifiable. Average certainty: **63%**.

---

## Headline issues for triage

1. **Ministry name on the page is correct but the page is miscategorised in the site architecture.** The content correctly names "Ministry of Youth, Sports and Community Empowerment" as the administering body, confirmed by every authoritative source. However, `src/data/ministries.ts` lists "Apply to be a Project Protégé Mentor" under `onlineServices` for the *Ministry of Labour, Social Security and Third Sector* — not under the Ministry of Youth, Sports and Community Empowerment where it belongs. This internal inconsistency could affect navigation and ministry landing-page listings. The page is also filed under the `work-employment` category slug rather than `youth-and-community`, which is a content architecture question for the team.

2. **YDP email address cannot be confirmed as an active inbox via a Tier 1 source.** The page states paper applications can be emailed to `YDP@barbados.gov.bb`. This address is cited in a gov.bb PDF document about the Youth Development Programme (a Tier 1 source) and was surfaced by web search. It cannot be independently confirmed via a live `.gov.bb` web page that is currently rendering. If this inbox is no longer monitored, citizens who submit paper applications by email may receive no acknowledgement.

3. **Four key claims about the post-interview process are unverifiable.** The safeguarding module, the orientation session, the ministry-led matching and networking event are all described on the page but do not appear in any public authoritative source. These are plausible operational details but cannot be independently confirmed from the web.
...
5. **`projectprotege.gov.bb` does not exist.** No dedicated portal for the programme was found. `projectprotege.gov.bb` returned ECONNREFUSED.

---

## Claims

### Claim 1 — Programme description and ministry name (index.md lines 1–9)
...
### Claim 5 — Ministry address: Sky Mall, Haggatt Hall, St Michael (index.md lines 26–31)
...
### Claim 6 — Ministry name: Ministry of Youth, Sports and Community Empowerment (index.md lines 26–27)
...
- **Status:** verified
- **Certainty:** 97%
- **Internal consistency issue:** Despite the content being correct, `src/data/ministries.ts` incorrectly lists "Apply to be a Project Protégé Mentor" under `onlineServices` for the *Ministry of Labour, Social Security and Third Sector* (lines 806–809), not under the Ministry of Youth, Sports and Community Empowerment. See [_internal-consistency.md](/home/gavin/frontend-alpha/docs/fact-check/_internal-consistency.md) for the ministry name variant record.

---

### Claim 7 — Submission to Youth Development Programme (index.md line 58)
...
### Claim 8 — YDP email address: YDP@barbados.gov.bb (index.md line 62)
...
<div class="claim-block claim-block--pending">
<div class="claim-block-label">Partially confirmed — live page not found, but Tier 1 document confirms</div>
...
However, the PDF cannot be fully rendered by this tool, and no currently
rendering .gov.bb web page explicitly lists this email address as active.
The youthaffairs.gov.bb contact page does not list any email address.</pre>
</div>

- **Type:** email
- **Sources consulted:** [gov.bb — Youth Development Programme PDF](https://www.gov.bb/media_files/YOUTH%20DEVELOPMENT%20PROGRAMME.pdf) — cited as the YDP contact; [youthaffairs.gov.bb — Contact Us](https://youthaffairs.gov.bb/contact-us/) — no email listed; web search snippet confirms "Completed forms should be emailed to YDP@barbados.gov.bb"
...
### Claim 9 — Police Certificate of Character link: forms.gov.bb/CertificateOfCharacter (index.md line 73)
...
### Claim 10 — Safeguarding module in training (index.md lines 77–78)
...
### Claim 11 — Orientation session before mentorship begins (index.md lines 81–82)
...
### Claim 12 — Ministry matches mentors to mentees; introduction at a networking event (index.md lines 83–84)
...
to commence the matching process" and that skills and interests are
considered. However, the specific mechanism of a "networking event" as the
point of introduction is not mentioned in any authoritative public source.</pre>
</div>
...
### Claim 13 — Form completion time: not more than 30 minutes (start.md line 11)
...
3. **No `source_url` in `content-directory.ts`.** The canonical source for this page is the youthaffairs.gov.bb launch article. Adding `source_url: "https://youthaffairs.gov.bb/youth-ministry-launches-mentorship-programme-project-protege/"` to the entry at `src/data/content-directory.ts` line 129 would make the provenance auditable.

4. **Ministry of Youth, Sports and Community Empowerment contact details not on the page.** The phone number (246) 535-3835 and email ps.mysce@barbados.gov.bb are the ministry's published contacts. The YDP phone is the same number. Useful for citizens with questions about application status.

5. **Page miscategorised under Ministry of Labour in ministries.ts.** `src/data/ministries.ts` lines 806–809 list this service under `onlineServices` for the Ministry of Labour, Social Security and Third Sector. Project Protégé is run by the Ministry of Youth, Sports and Community Empowerment. This entry should be moved to the MYSCE `onlineServices` block.
...
- [youthaffairs.gov.bb — About (Division of Youth & Culture)](https://youthaffairs.gov.bb/about-2/)
...
- [Barbados Today — Youth programme sees major expansion (March 2025)](https://barbadostoday.bb/2025/03/14/youth-programme-sees-major-expansion-2/amp/)
